...
## 3. STEP 1 — Bedrock(AI 모델) 활성화 테스트

AWS의 업데이트로 인해 이제 수동으로 모델 권한을 켤 필요 없이, 첫 호출 시 자동으로 모델이 활성화됩니다. 다만, AI 심사원으로 사용할 Claude 모델의 경우 최초 1회 플레이그라운드에서 직접 호출 테스트를 해야 안전하게 권한이 열립니다.
...
3. 왼쪽 메뉴에서 **`Playgrounds`** 하위의 **`Chat`** 클릭
4. **`Select model`** 클릭 → **`Anthropic`** 카테고리에서 **`Claude 3 Haiku`** 선택 후 **`Apply`**
   *(※ 만약 'Submit use case details' 창이 뜬다면, 화면의 안내에 따라 데모 및 사내 테스트 목적이라고 간단히 영문으로 기입 후 제출하세요.)*
5. 하단 채팅 입력창에 `안녕` 이라고 입력하고 **`Run`** 버튼을 클릭
6. 모델이 정상적으로 답변을 반환했다면, 이 계정 전체에 Claude 모델 사용 권한이 완벽하게 활성화된 것입니다! 완료!

> 이 단계를 건너뛰거나 응답을 받지 못하면, 이후 게임에서 "심사원과의 통신 중 오류" 가 발생할 수 있으니 꼭 한 번 답변을 받아보세요.
